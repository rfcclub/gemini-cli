...
- **WHEN** `~/.gemini-vesta/config.yaml` has
  `defaultModel: deepseek/deepseek-chat`
- **WHEN** `providers.yaml` has 5 providers registered
- **THEN** `resolveDefaultModel()` returns `ProviderModelId` where
  `provider = "groq"` AND `model = "llama-3.1-70b"`
- **THEN** the resolution source is tagged as `"env"` for logging.

#### Scenario: config.yaml.defaultModel overrides catalog order

- **WHEN** env `VESTA_DEFAULT_MODEL` is not set
- **WHEN** `~/.gemini-vesta/config.yaml` exists with
  `defaultModel: deepseek/deepseek-chat`
- **WHEN** `providers.yaml` has 5 providers where deepseek is not first
- **THEN** `resolveDefaultModel()` returns `ProviderModelId` where
  `provider = "deepseek"` AND `model = "deepseek-chat"`
- **THEN** the resolution source is tagged as `"config"`.

#### Scenario: catalog first provider used when no config

- **WHEN** env `VESTA_DEFAULT_MODEL` is not set
- **WHEN** `~/.gemini-vesta/config.yaml` does not exist
- **WHEN** `providers.yaml` exists with `providers[0] = groq` and
  `groq.availableModels[0] = llama-3.1-70b`
- **THEN** `resolveDefaultModel()` returns `ProviderModelId` where
  `provider = "groq"` AND `model = "llama-3.1-70b"`
- **THEN** the resolution source is tagged as `"catalog"`.

#### Scenario: fallback to first provider with warning when no sources

- **WHEN** env `VESTA_DEFAULT_MODEL` is not set
- **WHEN** `~/.gemini-vesta/config.yaml` does not exist
- **WHEN** `providers.yaml` has 1 provider with availableModels
...
- **WHEN** env `VESTA_DEFAULT_MODEL` is not set
- **WHEN** `~/.gemini-vesta/config.yaml` does not exist
- **WHEN** `providers.yaml` exists with zero providers
- **THEN** `resolveDefaultModel()` throws `NoProvidersConfiguredError`
- **THEN** error message contains the string
  `"No providers configured. Run \`vesta init-providers\` or set
  GEMINI_API_KEY."`.
...
- **THEN** logs `INFO: --provider flag active: deepseek`.
...
- **WHEN** ProviderRegistry initialization runs
- **THEN** `providerRegistry.getAllProviders()` does NOT include `"gemini"`
- **THEN** `providerRegistry.getProviderConfig("gemini")` returns `undefined`.
...
- **THEN** `providerRegistry.getProvider("gemini")` returns a provider object
...
- **THEN** `loadVestaConfig()` returns `VestaConfig` with all fields populated
  as parsed values
- **THEN** parsed YAML matches the Zod-style schema:
...
- **THEN** does not log any error or warning.
...
- **WHEN** `~/.gemini-vesta/config.yaml` exists with `geminiApiKey` field
  populated
- **WHEN** file permissions are `0644` (world-readable)
...
- **THEN** exits with status code 78 (`EX_CONFIG` from sysexits.h).
...
- **WHEN** `~/.gemini-vesta/providers.yaml` exists with ≥1 provider
- **WHEN** `~/.gemini-vesta/config.yaml` does not exist
- **THEN** `gemini-vesta --version` exits with status 0
- **THEN** `gemini-vesta --help` lists `--provider <name>` in available options.

#### Scenario: full boot sequence without GEMINI_API_KEY

- **WHEN** `GEMINI_API_KEY` env is unset
- **WHEN** `providers.yaml` exists with 1 provider
- **WHEN** config.yaml does not exist
- **THEN** invoking `gemini-vesta` with `--prompt "echo hi"` exits with status 0
- **THEN** the captured stdout/stderr contains the substring
