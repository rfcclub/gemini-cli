...
- **AND** the field is not required (no compile error if omitted)
...
- **AND** `getProviderConfig(name).availableModels` returns `undefined`
...
- **AND** the array order matches the YAML declaration order
...
- **AND** `availableModels` array is preserved without modification
...
- **AND** the registry state is preserved (no partial corruption)
...
- **AND** new tests for `availableModels` loading also pass
